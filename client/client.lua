...
Citizen.CreateThread(function()
    ...
    addBlipsForCateg(cfg.clothingShops, {73, 37, "Magazin de Haine"})
    addBlipsForCateg(cfg.barberShops, {71, 37, "Frizerie"})
    addBlipsForCateg(cfg.tattoosShops, {75, 37, "Salon Tatuaje"})
end)
...
AddEventHandler("clothes:firstSpawn", function()
    ...
    if IsModelInCdimage(model) and IsModelValid(model) then
        ...
        SetPedComponentVariation(PlayerPedId(), 6, 75, 2, 0)
        ...
        SetPedHeadOverlayColor(PlayerPedId(), 8, 15, 0, 0)
...
AddEventHandler('raid_clothes:incarcaHainele', function()
    LoadYourClothes()
    vRP.notify({"Succes: Ai folosit optiunea reload skin"})
end)
...
RegisterNUICallback('customskin', function(data, cb)
    -- Nothing.
    Citizen.Trace("Salut, aceasta functie a fost scoasa din magazin! O sa fie readaugata in 2042 sub pretul de 100 Bitcoins per try.")
end)
...
RegisterNUICallback('resetpedf', function(data, cb)
    SetSkin('mp_f_freemode_01', true)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if nearcloth < 5.0 then
            menu = {"clothesmenu", "Apasa ~INPUT_PICKUP~ pentru a-ti schimba ~b~hainele"}
        elseif neartat < 3.0 then
            menu = {"tattoomenu", "Apasa ~INPUT_PICKUP~ pentru a-ti schimba ~o~tatuajele"}
        elseif nearbarber < 5.0 then
            menu = {"barbermenu", "Apasa ~INPUT_PICKUP~ pentru a-ti schimba ~y~frizura"}
